//Author: ackerman
//Convert cart .chf to .h Arduino compile Fairchild Channel F emulate
//input
// cart
//  ColorOrgan.chf
//output
...
void WriteFileROM(unsigned char num,char *cadPath,char *cadFile, char *cadFileSource)
{
 ...
 sprintf(cadDest,"output/dataFlash/cart/gbCart%s.h",cadFile);
 auxWrite = fopen(cadDest,"w+");
 if (auxWrite!= NULL)
 {
  fprintf(auxWrite,"#ifndef _GB_CART_%s_H\n",cadFile);
  fprintf(auxWrite," #define _GB_CART_%s_H\n",cadFile);     
  ...
  if (auxRead!=NULL)
  {
   ...
   fprintf(auxWrite," //chr %s %d bytes\n\n",cadFile,auxSize);
   fprintf(auxWrite,"const unsigned char gb_cart_%s[]={\n",cadFile);
...
{//Los 48k
 ...
 for (unsigned char i=0;i<gb_contRom;i++)
 {
  ...
  {
   fprintf(gb_fileWrite,"  #include \"gbCart%s.h\"\n",gb_titles[i]);
  }
 } 
 ...
 for (unsigned char i=0;i<gb_contRom;i++)
 {
  if (strlen(gb_titles[i]) != 0)
  {
   fprintf(gb_fileWrite,"  #include \"cart/gbCart%s.h\"\n",gb_titles[i]);
  }
 }
 fprintf(gb_fileWrite," #endif\n");
 
 fprintf(gb_fileWrite,"\n");
 fprintf(gb_fileWrite," #define max_list_cart %d\n\n\n",gb_contRom);
 fprintf(gb_fileWrite," //chr\n //Titulos\n");
 fprintf(gb_fileWrite," static const char * gb_list_cart_title[max_list_cart]={\n");
 for (unsigned char i=0;i<gb_contRom;i++)
 {
  if (strlen(gb_titles[i]) == 0)
   fprintf(gb_fileWrite,"  \"\"");
 ...
 fprintf(gb_fileWrite,"\n };\n");
 fprintf(gb_fileWrite,"\n");
 fprintf(gb_fileWrite," //Datos chr\n");
 fprintf(gb_fileWrite," static const unsigned char * gb_list_cart_data[max_list_cart]={\n");
 for (unsigned char i=0;i<gb_contRom;i++)
 {
  if (strlen(gb_titles[i]) == 0)
   fprintf(gb_fileWrite,"  NULL");
  else 
   fprintf(gb_fileWrite,"  gb_cart_%s",gb_titles[i]);
  if (i<(gb_contRom-1))
   fprintf(gb_fileWrite,",\n");   
 }  
 fprintf(gb_fileWrite,"\n };\n");
...
void WriteSizeROM()
{
 ...
 fprintf(gb_fileWrite," static const int gb_list_cart_size[max_list_cart]={\n");
 for (unsigned char i=0;i<gb_contRom;i++)
 {
  if (strlen(gb_titles[i]) == 0)
   fprintf(gb_fileWrite,"  //\"\"");
  ...
  {
   sprintf(cadDestino,"input\\cart\\%s",gb_nameFiles[i]);   
...
{
 gb_contRom=0;
 gb_fileWrite = fopen("output/dataFlash/gbcart.h","w+");
 if (gb_fileWrite!= NULL)
 {
  listFilesRecursively("input/cart");
  WriteHeadROM_H("_GB_CART_H");
  WriteSizeROM();
  fprintf(gb_fileWrite,"#endif\n");
  fclose(gb_fileWrite);
 }     
